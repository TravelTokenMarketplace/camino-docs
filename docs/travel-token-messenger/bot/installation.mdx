...
__ToDo: update image after bot renaming has finished__
:::caution USE STABLE VERSION

When using our [GitHub repository](https://github.com/TravelTokenMarketplace/travel-token-messenger-bot),
...
![This image displays the TT Messenger bot repo tags](/img/messenger/tags.png)
...
This document guides you through the installation of the Travel token Messenger Bot
...
To get started, clone the Travel Token Messenger Bot repository and initialize the
submodules:

```sh
git clone -b c4t https://github.com/TravelTokenMarketplace/travel-token-messenger-bot.git
cd travel-token-messenger-bot
...
To ensure the build was successful, check for the `travel-token-messenger-bot` binary:

```
./build/travel-token-messenger-bot --help
```

Running `travel-token-messenger-bot --help` should produce the following output:

```
$ ./build/travel-token-messenger-bot --help
starts travel token messenger bot

Usage:
  travel-token-messenger-bot [flags]
...
```
$ ./build/travel-token-messenger-bot --version
travel-token-messenger-bot version v11.0.0 (git: 8555570)
...
```sh
docker pull c4tplatform/travel-token-messenger-bot:latest
```
...
First, clone the Travel Token Messenger Bot repository and initialize the
submodules:
...
```sh
docker build --tag travel-token-messenger-bot .
```

After a successful build, test the image with:

```sh
docker run --rm travel-token-messenger-bot --help
```
...
To enable the bot to connect to the Travel Token Messenger server, ensure that the network
...
The following known issues should be considered when using the Travel Token Messenger Bot:
...
- **Camino Messenger Account upgrade:** The CM Account has to be upgraded to the
  latest version, when upgrading the bot to v11.0.0 or later.
